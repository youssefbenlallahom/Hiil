...
export default function RneConversation({caseId,onChanged}:{caseId:string;onChanged:()=>Promise<void>}) {
  ...
  useEffect(()=>{if(history.current)history.current.scrollTop=history.current.scrollHeight;},[state?.messages.length,busy]);
  ...
  return <>
    ...
    <div className="rne-layout">
      <section className="panel rne-chat" aria-label="Conversation de préparation">
        ...
        <div className="rne-messages" ref={history} role="log" aria-live="polite" aria-relevant="additions">
          ...
          {busy&&<div className="rne-thinking" role="status"><LoaderCircle className="spin" size={16}/>{busy==='extract_cin'?'Lecture de la CIN et recherche des passages…':busy==='prepare'?'Remplissage du formulaire original…':busy==='message'?'L’assistant prépare sa réponse…':'Enregistrement…'}</div>}
        </div>
