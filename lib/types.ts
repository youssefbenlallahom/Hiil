...
export type PreparedField = {key:string;label:string;value:string;state:'confirmed'|'extracted'|'missing'|'conflict';evidence:(Fact & {document_id:string;document_name:string})[]};
export type Case = {id:string;company:string;title:string;sample:boolean;status:string;documents:Document[];confirmations:Record<string,{value:string;at:string;actor:string}>;events:{id:string;label:string;actor:string;detail:string;at:string}[];checks:{issues:Issue[];open_count:number;field_count:number;pending_documents:string[];scope:string};prepared_fields:PreparedField[];correction?:{note:string;at:string;pending:boolean;response:string|null};can_submit:boolean;updated_at:string};
